#!/bin/bash
# cd /home/edspeco/ultrasonic_classifier/ && bash script_3.sh
# cd /home/edspeco/ultrasonic_classifier/ && chmod 775 script_3.sh
...
# The value of FILE is set via 'export' in script_1.sh, line 55. Currently it is: /home/edspeco/ultrasonic_classifier/Final_result.txt
...
if [ -f "$FILE" ]; then
  ...
  echo $batName > '/home/edspeco/ultrasonic_classifier/helpers/status_update.txt'
  exec 6<&-

  cd /home/edspeco/ultrasonic_classifier/temp/

  newName=$(date +%d-%m-%Y_%H:%M:%S)
  cp $FILE /home/edspeco/ultrasonic_classifier/Final_result_copy.txt
...
#################################################################################################################
  ...
  # cp $FILE /home/edspeco/ultrasonic_classifier/results/${newName2}.txt        # Create the text file with new informative name.
#################################################################################################################


  cp ${newName2}.wav /home/edspeco/ultrasonic_classifier/detected_bat_audio/     # Copy the newly named wav file to detected bat audio folder.
  rm ${newName2}.wav
...
if [ $batConfidence3 -gt $conf ]; then
  if [ ${batName} = "HOUSE_KEYS" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/keys.wav
  elif [ ${batName} = "NOCTULA" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/noctule.wav
  elif [ ${batName} = "NATTERERI" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/nattereri.wav
  elif [ ${batName} = "PLECOTUS" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/plecotus.wav
  elif [ ${batName} = "NATTERERI" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/nattereri.wav
  elif [ ${batName} = "C_PIP" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/c_pip.wav
  elif [ ${batName} = "S_PIP" ]; then
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/s_pip.wav
  fi
fi
}

# printf "${GREEN}Is there still a  ./script_2.sh: 79: [: -gt: unexpected operator error? ${NC}\n"
directory_to_search_inside="/home/edspeco/ultrasonic_classifier/detected_bat_audio"
...
if [ $bat_detected -gt 0 ]; then
  ...
  if  [ $file_count -gt 500 ]; then
    cd /home/edspeco/ultrasonic_classifier/detected_bat_audio/
...
#################################################################################################################
  ...
  exec > /home/edspeco/ultrasonic_classifier/results/${newName2}.html        # Create the html file with new informative name.
  printf '<html>'
  printf '<head>'
