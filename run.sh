...
#!/bin/bash
# cd /home/edspeco/ultrasonic_classifier/ && bash run.sh
...
# sudo chmod -R 775 /home/edspeco/ultrasonic_classifier/
...
cat > /home/edspeco/ultrasonic_classifier/helpers/kernel.txt <<<$kernel
...
rm /home/edspeco/ultrasonic_classifier/helpers/batteryAlert.txt                           # Remove this file if previously there was battery fault.
...
if [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/close_app.txt" ]; then     # Check if there is a close_app.txt file from a previous session.
  echo "Run.sh reports: restart.txt file exists"
  rm /home/edspeco/ultrasonic_classifier/helpers/close_app.txt
...
do
  # echo "while loop"
  if [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/close_app.txt" ]; then     # Waiting for a 'close_app.txt' file to appear in 'helpers' folder.
  ...
  if [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/batteryAlert.txt" ]; then     # Look for batteryAlert.txt in 'helpers' folder.
  ...
  sleep 10
  aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/main_Computers_are_in_Control.wav
    ...
    aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/main_APU_Shutdown.wav
...
# ps -u edspeco       # Use this to see what shells are running under user = edspeco.
# ps -U root -u root -N
