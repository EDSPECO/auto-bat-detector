#!/bin/bash
# $ cd /home/edspeco/ultrasonic_classifier/ && bash script_1.sh
...
# command 2>&1 | tee ~/home/edspeco/ultrasonic_classifier/log.txt
exec > >(tee -i /home/edspeco/ultrasonic_classifier/log.txt)

amixer sset PCM 100%

# **** List of PLAYBACK Hardware Devices ****
# card 0: tegrahda [tegra-hda], device 3: HDMI 0 [HDMI 0]
aplay --device=hw:0,3 /home/edspeco/ultrasonic_classifier/alert_sounds/Go_for_Deploy.wav
# aplay --device=hw:0,0 /home/edspeco/ultrasonic_classifier/alert_sounds/Go_for_Deploy.wav

# sudo chmod 775 /home/edspeco/ultrasonic_classifier/helpers/toggled_01.txt

cd /home/edspeco/ultrasonic_classifier/

rm /home/edspeco/ultrasonic_classifier/temp/final.wav
rm /home/edspeco/ultrasonic_classifier/unknown_bat_audio/filtered.wav
rm /home/edspeco/ultrasonic_classifier/unknown_bat_audio/filtered.wav
rm /home/edspeco/ultrasonic_classifier/From_R_01.csv
rm Final_result.txt
rm Final_result_copy.txt
cd /home/edspeco/ultrasonic_classifier/helpers/

#rm stop.txt
rm start.txt
touch stop.txt
rm shutDown.txt
# rm /home/edspeco/ultrasonic_classifier/temp/*
rm /home/edspeco/ultrasonic_classifier/unknown_bat_audio/*
rm /home/edspeco/ultrasonic_classifier/results/*
...
export FILE=/home/edspeco/ultrasonic_classifier/Final_result.txt
...
while true
do
    while [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/stop.txt" ]; do          # This loop will block the classifier and recorder whilst waiting for a 'stop.txt' file to appear in 'helpers' folder.
		printf "script_1 reports: ${RED}stop.txt${NC} file exists\n"
		sleep 2
		echo "Status: STOPPED" > '/home/edspeco/ultrasonic_classifier/helpers/status_update.txt'
		if [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/shutDown.txt" ]; then     # Waiting for a 'shutDown.txt' file to appear in 'helpers' folder.
		...
		if [ -e "$1/home/edspeco/ultrasonic_classifier/helpers/restart.txt" ]; then     # Waiting for a 'restart.txt' file to appear in 'helpers' folder.
		  echo "script_1 reports: restart.txt file exists"
		  sleep 4
		  # rm /home/edspeco/ultrasonic_classifier/helpers/restart.txt
		  # exit
		fi
    done
	...
	value=`cat /home/edspeco/ultrasonic_classifier/helpers/toggled_01.txt`      # Toggled options include 'record' and 'process'.

	if [ "$value" == "record" ]; then
		# echo "script_1 reports: Recording! ....."
		echo "Started recording ..." > '/home/edspeco/ultrasonic_classifier/helpers/status_update.txt'
		# counter=`cat /home/edspeco/ultrasonic_classifier/helpers/counter.txt`
		# export iter=$counter
		cd /home/edspeco/ultrasonic_classifier/
		...
		echo "Started processing ..." > '/home/edspeco/ultrasonic_classifier/helpers/status_update.txt'
		sleep 2
		cd /home/edspeco/ultrasonic_classifier/
